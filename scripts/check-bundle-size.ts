 ...
 * (Pre- baseline was 44,581 B raw / 10,206 B gzip post-.)
 * Decision: re-baseline (not exemption). Rationale: the delta cap did its job
 * policing the fold-table landing; the table is now permanent baseline, so
 * measuring all future work against a pre-table baseline guarantees false
 * failures on the first commit. Rejected: exemption (keep old baseline,
 * gate on total only) — leaves a permanently-redundant check future agents
 * misread as live.
 * review hardening: multi-agent review found fail-open trust boundaries
 ...
 *
 * () document record engine expansion:
 * Adds DocumentIndex<TDoc>, field-stratified packing, scoreExactMatchesMultiField,
 ...
 *
 * () Unicode-safe highlighting engine expansion:
 ...
 *
 * () dynamic mutations & memory management expansion:
 ...
 *
 * () first-party worker client & protocol expansion:
 ...
 *
 * () snapshot persistence & IndexedDB storage expansion:
 * Adds persistence.ts (legacy snapshot 48-byte Little-Endian binary format, circular-dependency-free CRC32,
 ...
 * (+~7.5 KB gzip vs 30 KB cap) instead of conflating - + growth the
 * old -era baseline never absorbed (main was already +30,903 over it).
 * Total bumped 48 KB -> 60 KB with this documented rationale.
 * Budget: 60 KB gzip total per file (dist/index.js + dist/index.cjs).
 *
 * (facet aggregation) review hardening expansion:
 ...
 *
 * (snapshot persistence hardening + custom getter guard) review fixes:
 ...
 * Run: bun scripts/check-bundle-size.ts (or: node scripts/check-bundle-size.ts)
 */
