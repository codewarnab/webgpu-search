import { crc32Parts, validatePackedOffsets } from './dataset-packing';
import { collectHookIds } from './hooks';
import {
...
/** Snapshot reserved word must be zero (forward-extension guard). */
const SNAPSHOT_RESERVED_EXPECTED = 0;
...
/**
 * Encodes the columnar filter-attribute segment for a snapshot.
 *
...
/**
 * Validates a decoded columnar segment against the snapshot schema.
 ...
 * non-zero reserved words throw `IncompatibleIndexError`.
 *
...
export function decodeSnapshotHeader(buffer: ArrayBuffer): DocumentSnapshotHeader {
  ...
  if (magic === LEGACY_SNAPSHOT_MAGIC) {
    const dv = new DataView(buffer, 0, LEGACY_SNAPSHOT_HEADER_BYTES);
    const formatVersion = dv.getUint32(4, true);
    if (formatVersion !== LEGACY_SNAPSHOT_VERSION) {
      throw new IncompatibleIndexError(LEGACY_SNAPSHOT_VERSION, formatVersion);
    }
    ...
    if (foldedVal !== 0 && foldedVal !== 1) {
      throw new IncompatibleIndexError('normalized 0|1', foldedVal);
    }
  ...
  if (magic === SNAPSHOT_MAGIC) {
    if (byteLen < SNAPSHOT_HEADER_BYTES) {
      throw new IncompatibleIndexError(SNAPSHOT_MAGIC, 'neutered/short-u2d4');
    }
    const dv = new DataView(buffer, 0, SNAPSHOT_HEADER_BYTES);
    const formatVersion = dv.getUint32(4, true);
    if (formatVersion !== SNAPSHOT_FORMAT_VERSION) {
      throw new IncompatibleIndexError(SNAPSHOT_FORMAT_VERSION, formatVersion);
    }
    const profileEnum = dv.getUint32(8, true);
    const unicodeVersionEnum = dv.getUint32(12, true);
    const scoringVersionEnum = dv.getUint32(16, true);
    ...
    if (reserved !== SNAPSHOT_RESERVED_EXPECTED) {
  ...
  throw new IncompatibleIndexError(`${LEGACY_SNAPSHOT_MAGIC}|${SNAPSHOT_MAGIC}`, magic);
}

/**
 * Serializes a DocumentIndex into a versioned snapshot Little-Endian binary ArrayBuffer.
 *
 ...
 */
export function encodeSnapshot<TDoc = Record<string, unknown>>(
  index: DocumentIndex<TDoc>,
  options?: SerializeDocumentIndexOptions
): ArrayBuffer {
  if (!index) {
    throw new TypeError('[webgpu-search] encodeSnapshot requires a DocumentIndex instance.');
  }
  ...
  const normalized = index.isNormalized();
  ...
  const schema: DocumentIndexSchema = {
    ...
    docIds: docIds.slice(),
    caseSensitive: index.options.caseSensitive ?? !normalized,
  ...
  // Columnar filter-attribute segment.
  ...
  dv.setUint32(4, SNAPSHOT_FORMAT_VERSION, true);
  ...
  const payloadSchema = new Uint8Array(out, SNAPSHOT_HEADER_BYTES, schemaByteLength);
  const payloadTokens = new Uint8Array(out, SNAPSHOT_HEADER_BYTES + schemaByteLength, tokensByteLength);
  const payloadOffsets = new Uint8Array(out, SNAPSHOT_HEADER_BYTES + schemaByteLength + tokensByteLength, offsetsByteLength);
  const payloadColumnar = columnarByteLength > 0
    ? new Uint8Array(out, SNAPSHOT_HEADER_BYTES + schemaByteLength + tokensByteLength + offsetsByteLength, columnarByteLength)
    : new Uint8Array(0);
  const payloadDocs = docsByteLength > 0
    ? new Uint8Array(out, SNAPSHOT_HEADER_BYTES + schemaByteLength + tokensByteLength + offsetsByteLength + columnarByteLength, docsByteLength)
    : new Uint8Array(0);
 ...
 * Accepts canonical and legacy snapshots (migration read path).
...
export function decodeSnapshot<TDoc = Record<string, unknown>>(
  ...
  const isCanonical = header.magic === SNAPSHOT_MAGIC;
  const headerBytes = isCanonical ? SNAPSHOT_HEADER_BYTES : LEGACY_SNAPSHOT_HEADER_BYTES;
  const headerPrefixLen = isCanonical ? 52 : 44;
  ...
  // Fail-closed columnar shape validation (authoritative rebuild via
...
/** @deprecated Use encodeSnapshot. Bytes unchanged. */
export const serializeDocumentIndex = encodeSnapshot;
/** @deprecated Use decodeSnapshot. Bytes unchanged. */
export const deserializeDocumentSnapshot = decodeSnapshot;
/** @deprecated Use decodeSnapshotHeader. Bytes unchanged. */
export const deserializeDocumentSnapshotHeader = decodeSnapshotHeader;
/** @deprecated Use restoreSnapshot. */
export const restoreDocumentIndex = restoreSnapshot;
