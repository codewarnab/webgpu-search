/**
 * v0.4 extensibility pipeline (Issue #10 M6).
 *
...
import { isTokenDelimiter, splitQueryTerms } from './modes/token-search';
import type {
 ...
 */
export function normalizeSearchExtensionHooks<TDoc>(
  raw: SearchExtensionHooks<TDoc> | undefined
): SearchExtensionHooks<TDoc> | undefined {
  ...
  const out: SearchExtensionHooks<TDoc> = {};
  let count = 0;
...
export function resolveEffectiveHooks<TDoc>(
  indexHooks: SearchExtensionHooks<TDoc> | undefined,
  queryHooks: SearchExtensionHooks<TDoc> | undefined
): SearchExtensionHooks<TDoc> | undefined {
  const normIndex = normalizeSearchExtensionHooks(indexHooks);
  const normQuery = normalizeSearchExtensionHooks(queryHooks);
...
export function hasAnyHook<TDoc>(hooks: SearchExtensionHooks<TDoc> | undefined): boolean {
...
export function collectHookIds<TDoc>(
  hooks: SearchExtensionHooks<TDoc> | undefined
 ...
 * compatible with pre-M6 snapshots).
 */
export function assertHooksSatisfied<TDoc>(
  required: ExtensionHookIds | undefined,
  provided: SearchExtensionHooks<TDoc> | undefined
): void {
 ...
 * hook over the raw query, each term normalized post-fold) or the default
 * `splitQueryTerms` path over post-fold code points. Empty hook output yields
...
export function getTokenTermsForQuery(
  ...
  for (let i = 0; i < terms.length; i++) {
    const norm = normalizeText(terms[i] as string, folded);
