/**
 * Dataset packing, string normalization, and WebGPU memory limit validation.
 * Exact path packs post-normalization u32 scalars (no locale/UTF-16 helpers).
...
/** @deprecated Use PackedDataset. */
export type PackedUnicodeBufferV2 = PackedDataset;

export interface DatasetPackOptions {
  normalized?: boolean;
  /** @deprecated Use normalized. */
  folded?: boolean;
  profileId?: TextProfileId;
...
/** @deprecated Use DatasetPackOptions. */
export type UnicodePackOptions = DatasetPackOptions;
...
/**
 * Legacy sanitizer for the GPU byte path (not the exact path).
 * @deprecated The exact path uses `normalizeText()` normalized tokens. Kept for migration only.
 */
export function sanitizeStringForSlot(str: string, maxChars?: number): string {
  if (!warnedLegacySanitizer) {
    ...
    console.warn(
      '[webgpu-search] sanitizeStringForSlot (legacy-ascii) is deprecated. Use normalizeText instead.'
    );
  }
  if (!str) return '';
...
export function packStringsToGPUBuffer(
  ...
  console.warn(
    '[webgpu-search] packStringsToGPUBuffer (legacy-ascii) is deprecated. Use packDataset instead.'
  );
  warnedLegacySanitizer = true;
...
/**
 * Pack post-normalization u32 scalars (exact path). Accepts pre-tokenized
...
export function packDataset(
  ...
  const normalized = options.normalized ?? options.folded ?? true;
  ...
  if (rowCount === 0) {
    ...
    return {
      tokens, offsets, rowCount: 0, tokenCount: 0, normalized, folded, profileId,
  ...
  return {
    tokens, offsets, rowCount, tokenCount, normalized, folded, profileId,
    unicodeVersion, nfcProbedVersion: null, scoringVersion,
    formatVersion: DATASET_FORMAT_VERSION,
    recordsBufferData: tokens.buffer as ArrayBuffer,
    offsetsBufferData: offsets.buffer as ArrayBuffer,
...
export function serializeDataset(packed: PackedDataset): ArrayBuffer {
  ...
  h.set([DATASET_MAGIC, DATASET_FORMAT_VERSION, pe, ue, se, packed.rowCount, packed.tokenCount, packed.folded ? 1 : 0, 0]);
...
export function deserializeDataset(buffer: ArrayBuffer): PackedDataset {
  ...
  if (!Number.isFinite(byteLen) || byteLen === 0 || !canSlice) {
    throw new IncompatibleIndexError(DATASET_MAGIC, 'neutered/empty');
  }
  const ab = buffer as ArrayBuffer;
  if (byteLen < 36 || byteLen % 4 !== 0) {
    throw new IncompatibleIndexError(DATASET_MAGIC, 'bad-length');
  }
  const h = new Uint32Array(ab, 0, 9);
  const magic = h[0];
  if (magic !== DATASET_MAGIC) throw new IncompatibleIndexError(DATASET_MAGIC, magic);
  if ((h[1]) !== DATASET_FORMAT_VERSION) throw new IncompatibleIndexError(DATASET_FORMAT_VERSION, h[1]);
  ...
  if ((h[7]) !== 0 && (h[7]) !== 1) {
    throw new IncompatibleIndexError('normalized 0|1', h[7]);
  }
  const folded = (h[7]) === 1;
...
/** @deprecated Use packDataset. */
export const packUnicodeToGPUBuffer = packDataset;
/** @deprecated Use serializeDataset. Wire format unchanged. */
export const serializeUnicodeDataset = serializeDataset;
/** @deprecated Use deserializeDataset. Wire format unchanged. */
export const deserializeUnicodeDataset = deserializeDataset;
