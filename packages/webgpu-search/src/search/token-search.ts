 ...
 *
 * Portable: no DOM refs. Operates on post-normalization u32 token streams.
 */
 ...
 * stays intact (code-aware splitting lands with custom tokenizer hooks).
 */
...
export function normalizeTokenMatchOptions(
  ...
  const minMatch = raw.minMatchCount;
  if (typeof minMatch !== 'number' || !Number.isInteger(minMatch) || minMatch < 1) {
    throw new RangeError(
      `[webgpu-search] tokenMatch.minMatchCount must be an integer >= 1, got ${String(minMatch)}.`
    );
  }
  return { operator, minMatchCount: minMatch };
}
