...
### M4 — Parity, fallback, CI (revised pre-implementation — accounts for M0–M3 as-landed via #15/#16/#17)
...
> M4 delivery: `scripts/test-m4-parity.ts` (110 asserts green, 1 pending-hardware canary; `bun run test:parity`, wired into CI), `test-vgpu-mock.ts` §25 sentinels (EXTEND, no rewrite), `test-regression.ts` M4 browser parity block (release gate), worker blocker migrated (`search.worker.ts` unicode `LOAD_DATASET` + `STRING_ISOLATED_ENRICHMENT`, `dataset.ts` U2F2, `main.ts` transfer + enrichment). Library `src/` untouched (bundle `21374 B` gzip vs `22528 B` budget, delta `+3031 B` vs `+4096 B` cap). Exact GPU-order cells stay `pending-hardware` on `vgpu/mock` (never executes WGSL); `test:browser` is the release gate.
